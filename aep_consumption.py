...
# Saatlik veriler için rastgele bir zaman serisi verisi
df = pd.read_csv('AEP_hourly.csv')  # Saatlik veriler için dosyanın ismini uygun şekilde değiştirdiğinizden emin olun

df = df[['Datetime', 'AEP_MW']]
df.columns = ['ds', 'y']

# Tarih sütununu datetime formatına dönüştürme (saatlik veriler için)
df['ds'] = pd.to_datetime(df['ds'])

# Son 10 saatlik veriyi tutma
forecast_steps = 10
...
future = prophet.make_future_dataframe(periods=forecast_steps, freq='H')  # Saatlik frekans
...
forecast_dates = [df["ds"].iloc[-1] + timedelta(hours=i) for i in range(1, forecast_steps + 1)]
...
results["Date"] = results["Date"].dt.strftime('%H:%M')
...
def display_table_and_graph():
    ...
    ax.set_title("American Electric Power Consumption")
    ax.set_xlabel("Date")
